%One often gains a better understanding of a concept by ``seeing'' it. % For instance, one can study the function $f(x)=x^2$ and describe many properties of how the output relates to the input without producing a graph, but the graph can quickly bring meaning and insight to equations and formulae. Not only that, but the study of graphs of functions is in itself a wonderful mathematical world, worthy of exploration.
%
%This chapter explores the basics of understanding vectors and matrix ``behavior'' graphically. Visualizing vectors can give deeper meaning to the concepts learned previously in this book, and opens the door to a grand new world that contains many important concepts. For instance, the concepts we learn in the chapter form part of the basis for computer graphics. While we often think of computer graphics in terms 3D games, the same principles are used to help chemists and biologists ``see'' complex molecules and understand their interactions with other molecules.
%
%This chapter is filled with figures. Be careful to look at the figure labels to make sure you associate the right picture with the right concept. Also, to avoid clutter, often the scales on the axes are not given. Unless it is stated otherwise, we will let one tick mark represent one unit of length. 

%It is good to visualize ... warning, pay attention to the Figure statements to see where things are supposed to be. unless stated otherwihse, tick marks signify one unit.

\section{Visualizing Matrix Arithmetic in 2D}\label{sec:geom_1}

%% If this becomes its own chapter, the following, edited a bit,
%% could become the preface to the chapter.

%This chapter began by making an analogy; we are studying operations on matrices that are similar to operations on real numbers. Given numbers $a$ and $b$, we know what $a+b$ means and what $3a$ means, and we now know what the matrix operations $\tta+\ttb$ and $3\tta$ mean. In learning about addition and multiplication on real numbers, it was useful to learn about the number line
...
\item	T/F: Mutliplying a vector by a  matrix always changes its length and direction
}
...
%We now investigate a way to picture matrix vectors. There are two benefits to this. First, it helps us understand matrix arithmetic operations. Secondly, as mentioned in the introduction to this chapter, it opens the door to a great wealth of interesting study.
...
%In this section, we'll restrict our attention to a special type of matrix, called a \textit{vector}. We define it formally below, followed by some explanations and examples.
%
%\definition{def:vector}{\index{vector!column}\index{vector!row}\textbf{Matrix Vectors}\\
%
%A $m\times 1$ matrix is called a \textit{column vector}. \\
%
%A $1\times n$ matrix is called a \textit{row vector}.}
%
%While it isn't obvious right now, column vectors are going to become far more useful to us than row vectors.\footnote{In this text, row vectors are only used in the next section when we discuss matrix multiplication (which we haven't learned about yet -- that is why it is in the \textit{next} section.) Other texts make great use of row vectors, but little use of column vectors. It is a matter of preference and tradition: ``most'' texts use column vectors more.} Therefore, we usuall omit the word ``column'' when refering to column vectors, and we just call them ``vectors.'' 
%
...
\begin{figure}[h!]
\begin{center}
\begin{tikzpicture}%[x={(.5cm,0)},y={(0,.5cm)}]
\draw (-3,0)--(3,0);
\draw (0,-3)--(0,3);
\foreach \x in {-2,...,2}
 {\draw  (\x,-.1)--(\x,.1);
...
%\begin{figure}
%\begin{center}\includegraphics[width=.3\linewidth]{MA_103_MetaPost_Figures/05_vectors_1}\end{center}
%\caption{Drawings of the vector $\vx = \bmx{c}1\\2\emx$.}
%\caption{Various drawings of $\protect\vx$}
%\label{fig:draw_vectors}
%\end{figure}
...
The picture in Figure \ref{fig:addvectors2} illustrates this. The gray vectors demonstrate drawing the second vector from the tip of the first; we draw the vector $\vx+\vy$ in red to set it apart from the rest. We also filled with a light blue the \textit{parallelogram} whose opposing sides are the vectors \vx\ and \vy. This highlights what is known as the \textit{Parallelogram Law}\index{Parallelogram Law}.
...
\draw[->,thick,red] (0,0)--(4,2) node [above right, black] {$\vx+\vy$};
...
\draw[->,thick, red] (0,0)--(-1,2) node[above, black]{$\vx+\vy$};
\etz
...
\draw[->, thick, red] (0,0)--(2,-.5) node[right,black] {\vx-\vy};
...
\draw[->, thick] (0,0)--(-1,1) node[above] {\vy};
\draw[->, thick] (0,0)--(1,-1) node[below] {-\vy};
\draw[->, thick, blue] (-1,1)--(1,.5) node[above,pos=.9,black] {$\vx-\vy$};
\draw[->, thick, red] (0,0)--(2,-.5) node[right,black] {$\vx-\vy$};
...
\draw[->, thick] (0,0)--(1,1) node[above] {\vy};
\draw[->, thick] (0,0)--(1,-1) node[below] {\vx};
\draw[->, thick, red] (1,1)--(1,-1) node[pos=.8,right,black] {$\vx-\vy$} ;
\etz
...
\draw[red,thick] (0,1)--(4,1);
\draw[thick,blue] (4,1)--(4,4);
...
\definition{def:vector_length}{\textbf{Vector Length}\\
